...
void LinWriting() {
  LinWrite(0x03, sending);
  Answer();
  delay(1000);
}

void LinResponding() {
  LinResponse(0x01, sending);
  Answer();
}
...
void LinResponse(byte PID, byte* sending) {
  if (1<<FE1) {
    delayMicroseconds(frameerrordelay);                 //after Frame Error Tbit to Sync Field
    if (Serial1.available() > 0) {
      sync = Serial1.read();
      if (sync == 0x55) {
        ...
        if (Serial1.available() > 0) {
          idbyte = Serial1.read();
          if ( (idbyte & 0x3F) == PID && idbyte>>6 == PIDCRC(idbyte) ) {
            delayMicroseconds(responsespace);           //after PID Tbit space
            for (int i=0;i<numbers;i++) {
...
void LinWrite(byte PID,byte* sending) {
  ...
  Serial1.begin(linspeed);
  Serial1.write(0x55);
  Serial1.write(PID | PIDCRC(PID)<<6 );
